...
THEME="$HOME/.config/rofi/wifi/wifi.rasi"
...
rofi_list() {
  local prompt="$1"
  shift
  exec rofi -dmenu -p "$prompt" -theme "$THEME" \
    -theme-str 'mainbox { children: [listview]; }' \
    "$@"
}

# With search bar — network list only
# Note: each -theme-str is separate to avoid multi-line parse issues
rofi_search() {
  local prompt="$1"
  shift
  rofi -dmenu -i -p "$prompt" -theme "$THEME" \
    -theme-str 'mainbox { children: [inputbar, listview]; spacing: 8px; }' \
    -theme-str 'sep { background-color: #4c566a; height: 1px; margin: 0px 4px; }' \
...
get_status() {
  ...
  local row
  row=$(nmcli -t -f ACTIVE,SSID,SIGNAL dev wifi --rescan no 2>/dev/null |
    grep '^yes:' | head -1)
  [ -z "$row" ] && return

  CONNECTED=true
  ACTIVE_SSID=$(echo "$row" | cut -d: -f2)
  ACTIVE_SIG=$(echo "$row" | cut -d: -f3)
}
...
do_scan() {
  ...
  local chosen
  chosen=$(echo "$networks" | rofi_search "󰤨 Networks")
  [ -z "$chosen" ] && return
...
main() {
  ...
  if $WIFI_ON; then
    ...
    opts+=("󰆴  Forget Network")
    ((i++))
    opts+=("󰖪  Turn Off WiFi")
    urgent_idx+=($i)
  else
    opts+=("󰖩  Turn On WiFi")
  fi
  ...
  local u_flag="" a_flag=""
  [ ${#urgent_idx[@]} -gt 0 ] && u_flag="-u $(
    IFS=,
    echo "${urgent_idx[*]}"
  )"
  [ ${#active_idx[@]} -gt 0 ] && a_flag="-a $(
    IFS=,
    echo "${active_idx[*]}"
  )"

  local chosen
  chosen=$(printf "%s\n" "${opts[@]}" | rofi_list "$prompt" $u_flag $a_flag)

  case "$chosen" in
  *"Scan & Connect"*) do_scan ;;
  *"Details"*) show_details ;;
  *"Disconnect"*) do_disconnect ;;
  *"Forget"*) do_forget ;;
  *"Turn Off WiFi"*) nmcli radio wifi off && notif "WiFi" "Turned off" ;;
